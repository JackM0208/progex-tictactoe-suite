...
async function sendMove(index){
    ...
    const response = await fetch('http://localhost:8080/ttt-servlet/play', {
        method: 'POST',
        headers: {
    ...
    console.log("Raw response from server:", rawText);  // print it
    const data = JSON.parse(rawText);  // then parse manually
